package Pensiones.Datospensionados;

public class DatosP33 {

    private String AP;
    private String AM;
    private String Name;
    private String Direccion;
    private String Mail;
    private String Placas1;
    private String Marca1;
    private String Modelo1;
    private String Color1;
    private String Ano1;
    private String Importe1;
    private String Recargo1;
    private String IVA1;
    private String Placas2;
    private String Marca2;
    private String Modelo2;
    private String Color2;
    private String Ano2;
    private String Importe2;
    private String Recargo2;
    private String IVA2;
    private String Placas3;
    private String Marca3;
    private String Modelo3;
    private String Color3;
    private String Ano3;
    private String Importe3;
    private String Recargo3;
    private String IVA3;
    private String TCasa;
    private String TOfi;
    private String Tcel;
    private String Observacioenes;
    private String ClaseDA;
    private String TDpension;
    private String Status;
    private String MesPago;
    private String RazonS;
    private String Npadron;
    private String Diainicio;
    private String Mesinicio;

    public String getAP() {
        return AP;
    }

    public void setAP(String AP) {
        this.AP = AP;
    }

    public String getAM() {
        return AM;
    }

    public void setAM(String AM) {
        this.AM = AM;
    }

    public String getName() {
        return Name;
    }

    public void setName(String Name) {
        this.Name = Name;
    }

    public String getDireccion() {
        return Direccion;
    }

    public void setDireccion(String Direccion) {
        this.Direccion = Direccion;
    }

    public String getMail() {
        return Mail;
    }

    public void setMail(String Mail) {
        this.Mail = Mail;
    }

    public String getPlacas1() {
        return Placas1;
    }

    public void setPlacas1(String Placas1) {
        this.Placas1 = Placas1;
    }

    public String getMarca1() {
        return Marca1;
    }

    public void setMarca1(String Marca1) {
        this.Marca1 = Marca1;
    }

    public String getModelo1() {
        return Modelo1;
    }

    public void setModelo1(String Modelo1) {
        this.Modelo1 = Modelo1;
    }

    public String getColor1() {
        return Color1;
    }

    public void setColor1(String Color1) {
        this.Color1 = Color1;
    }

    public String getAno1() {
        return Ano1;
    }

    public void setAno1(String Ano1) {
        this.Ano1 = Ano1;
    }

    public String getImporte1() {
        return Importe1;
    }

    public void setImporte1(String Importe1) {
        this.Importe1 = Importe1;
    }

    public String getRecargo1() {
        return Recargo1;
    }

    public void setRecargo1(String Recargo1) {
        this.Recargo1 = Recargo1;
    }

    public String getIVA1() {
        return IVA1;
    }

    public void setIVA1(String IVA1) {
        this.IVA1 = IVA1;
    }

    public String getPlacas2() {
        return Placas2;
    }

    public void setPlacas2(String Placas2) {
        this.Placas2 = Placas2;
    }

    public String getMarca2() {
        return Marca2;
    }

    public void setMarca2(String Marca2) {
        this.Marca2 = Marca2;
    }

    public String getModelo2() {
        return Modelo2;
    }

    public void setModelo2(String Modelo2) {
        this.Modelo2 = Modelo2;
    }

    public String getColor2() {
        return Color2;
    }

    public void setColor2(String Color2) {
        this.Color2 = Color2;
    }

    public String getAno2() {
        return Ano2;
    }

    public void setAno2(String Ano2) {
        this.Ano2 = Ano2;
    }

    public String getImporte2() {
        return Importe2;
    }

    public void setImporte2(String Importe2) {
        this.Importe2 = Importe2;
    }

    public String getRecargo2() {
        return Recargo2;
    }

    public void setRecargo2(String Recargo2) {
        this.Recargo2 = Recargo2;
    }

    public String getIVA2() {
        return IVA2;
    }

    public void setIVA2(String IVA2) {
        this.IVA2 = IVA2;
    }

    public String getPlacas3() {
        return Placas3;
    }

    public void setPlacas3(String Placas3) {
        this.Placas3 = Placas3;
    }

    public String getMarca3() {
        return Marca3;
    }

    public void setMarca3(String Marca3) {
        this.Marca3 = Marca3;
    }

    public String getModelo3() {
        return Modelo3;
    }

    public void setModelo3(String Modelo3) {
        this.Modelo3 = Modelo3;
    }

    public String getColor3() {
        return Color3;
    }

    public void setColor3(String Color3) {
        this.Color3 = Color3;
    }

    public String getAno3() {
        return Ano3;
    }

    public void setAno3(String Ano3) {
        this.Ano3 = Ano3;
    }

    public String getImporte3() {
        return Importe3;
    }

    public void setImporte3(String Importe3) {
        this.Importe3 = Importe3;
    }

    public String getRecargo3() {
        return Recargo3;
    }

    public void setRecargo3(String Recargo3) {
        this.Recargo3 = Recargo3;
    }

    public String getIVA3() {
        return IVA3;
    }

    public void setIVA3(String IVA3) {
        this.IVA3 = IVA3;
    }

    public String getTCasa() {
        return TCasa;
    }

    public void setTCasa(String TCasa) {
        this.TCasa = TCasa;
    }

    public String getTOfi() {
        return TOfi;
    }

    public void setTOfi(String TOfi) {
        this.TOfi = TOfi;
    }

    public String getTcel() {
        return Tcel;
    }

    public void setTcel(String Tcel) {
        this.Tcel = Tcel;
    }

    public String getObservacioenes() {
        return Observacioenes;
    }

    public void setObservacioenes(String Observacioenes) {
        this.Observacioenes = Observacioenes;
    }

    public String getClaseDA() {
        return ClaseDA;
    }

    public void setClaseDA(String ClaseDA) {
        this.ClaseDA = ClaseDA;
    }

    public String getTDpension() {
        return TDpension;
    }

    public void setTDpension(String TDpension) {
        this.TDpension = TDpension;
    }

    public String getStatus() {
        return Status;
    }

    public void setStatus(String Status) {
        this.Status = Status;
    }

    public String getMesPago() {
        return MesPago;
    }

    public void setMesPago(String MesPago) {
        this.MesPago = MesPago;
    }

    public String getRazonS() {
        return RazonS;
    }

    public void setRazonS(String RazonS) {
        this.RazonS = RazonS;
    }

    public String getNpadron() {
        return Npadron;
    }

    public void setNpadron(String Npadron) {
        this.Npadron = Npadron;
    }

    public String getDiainicio() {
        return Diainicio;
    }

    public void setDiainicio(String Diainicio) {
        this.Diainicio = Diainicio;
    }

    public String getMesinicio() {
        return Mesinicio;
    }

    public void setMesinicio(String Mesinicio) {
        this.Mesinicio = Mesinicio;
    }

    public String getDiaFin() {
        return DiaFin;
    }

    public void setDiaFin(String DiaFin) {
        this.DiaFin = DiaFin;
    }

    public String getMesfin() {
        return Mesfin;
    }

    public void setMesfin(String Mesfin) {
        this.Mesfin = Mesfin;
    }

    public String getTapagar() {
        return Tapagar;
    }

    public void setTapagar(String Tapagar) {
        this.Tapagar = Tapagar;
    }

    public String getTpagado() {
        return Tpagado;
    }

    public void setTpagado(String Tpagado) {
        this.Tpagado = Tpagado;
    }

    public String getFaltante() {
        return faltante;
    }

    public void setFaltante(String faltante) {
        this.faltante = faltante;
    }
    private String DiaFin;
    private String Mesfin;
    private String Tapagar;
    private String Tpagado;
    private String faltante;

}
